    ...
    <x-slot name="header">
        <div class="pb-4 border-b border-[#003a8c]/20">
            <h2 class="text-3xl font-bold text-[#003a8c] font-serif">
                {{ $Secsubject->name }} Learning Materials
            </h2>
            <p class="mt-2 text-gray-600">Manage and access all teaching resources for this subject</p>
        </div>
    </x-slot>

    <div class="px-4 sm:px-8 md:px-16 lg:px-24 py-6 space-y-8">
        @if ($Secmaterials->isEmpty())
            <div class="text-center mt-36 p-8 max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden border border-[#003a8c]/20">
            ...
            <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-[#003a8c]/10 mt-24">
                <div class="grid grid-cols-12 px-6 py-3 bg-[#003a8c] text-yellow-300 font-medium text rounded-t-xl">
                    <div class="col-span-6 flex items-center space-x-2">
                        ...
                        <span class="text-base font-semibold">Available Materials ({{ $Secmaterials->count() }})</span>
                    </div>
                ...
                <ul class="divide-y divide-[#003a8c]/10">
                    @foreach ($Secmaterials as $material)
                        @php
                            ...
                            <div class="grid grid-cols-12 items-center px-6 py-4">
                                <div class="col-span-6 flex items-center min-w-0">
                                    ...
                                    <div class="ml-4 min-w-0">
                                        <a href="{{ $fileUrl }}" class="text-lg font-medium text-[#003a8c] hover:underline truncate block" target="_blank" title="{{ basename($material->file_path) }}">
                                ...
                                <div class="col-span-2 flex justify-end items-center mx-4">
                                    <form action="{{ route('tutor.material.delete', $material->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this file?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="inline-flex items-center px-4 py-2 bg-red-600 text-white hover:bg-red-700 rounded-lg text-sm font-semibold shadow-sm transition-all">
                                            Delete
                                        </button>
                                    </form>
                                </div>
        ...
        <a href="{{ route('tutor.secondary.upload', ['slug' => $Secsubject->slug]) }}"
   class="inline-flex items-center px-6 py-3 bg-[#003a8c] text-yellow-300 hover:bg-yellow-300 hover:text-[#003a8c] rounded-lg font-bold">
